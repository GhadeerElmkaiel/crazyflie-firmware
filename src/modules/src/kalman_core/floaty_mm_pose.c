...
void floatyKalmanCoreUpdateWithPose(floatyKalmanCoreData_t* thi_s, poseMeasurement_t *pose)
{
  ...
  for (int i=0; i<3; i++) {
    // h[FKC_STATE_X+i] = 1;
    if(use_delay_comp==1){
      // value = pose->pos[i] +dt*(prev_vel[i]+bef_prev_vel[i]+bef_2_prev_vel[i]);
      value = pose->pos[i] +dt*(prev_vel[i]+bef_prev_vel[i]);

      floatyKalmanCoreScalarUpdateDiagP(thi_s, i, value - thi_s->S[FKC_STATE_X+i], measurementPosNoiseStd);
      ...
      floatyKalmanCoreScalarUpdateDiagP(thi_s, i, pose->pos[i] - thi_s->S[FKC_STATE_X+i], measurementPosNoiseStd);
    }
    // h[FKC_STATE_X+i] = 0;
  }
